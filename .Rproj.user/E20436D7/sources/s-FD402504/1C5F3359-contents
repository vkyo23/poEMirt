...
#' @param quiet A bool. Suppress messages (TRUE) or not. Default is FALSE.
...
read_poEMirt <- function(data, 
  ...
  excl <- data %>% 
    ...
    if (nrow(excl) > 0) {
      if (!quiet) {
        message('* Remove following items due to no variation in responses')
        message('  -', excl$j)
      }
      data <- data %>% 
        dplyr::filter(
  ...
  dimnames(Y)[[3]] <- Ks
  if (!quiet) message('* Dimension of the response array: ', paste(I, 'x', J, 'x', maxK))
