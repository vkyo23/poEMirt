...
for (i in 1:I) {
  mis <- sample(0:20, 1)
  if (mis != 0) {
    mis <- sample(1:39, size = mis)
    timemap[i, mis] <- 0
  }
}
timemap2 <- matrix(0, I, T)
for (i in 1:I) {
  wh <- which(timemap[i, ] == 1)
  if (length(wh) != 0) {
    timemap2[i, min(wh):T] <- 1
  }
}
...
# Load funcitons
rm(list = ls()[!ls() %in% c('d', 'con', 'theta', 'alpha', 'beta', 'timemap', 'timemap2')])
...
unc <- poEMirt_uncertainty(
  ...
  control = list(
    ...
    thin = 1,
    PG_approx = TRUE,
    save_item_parameters = FALSE
  )
)
unc$parameter$theta
gibbs <- summary(unc, parameter = 'theta')
