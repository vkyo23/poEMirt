...
poEMirt_uncertainty <- function(fit, 
                                ...
                                control = NULL) 
{
  if (class(fit) != 'poEMirtFit') stop('`fit` must be "poEMirtFit" object from poEMirt::poEMirt().')
  ...
  if (method == 'bootstrap') {
    ...
    if (!exists('save_item_parameters', control)) control$save_item_parameters <- FALSE
    ...
    if (control$PG_approx) {
      meann <- mean(fit$info$data$trial)
      if (meann < 10) {
        message('* WARNING: `PG_approx = TRUE` is not recommended for small number of trials (e.g., n < 10)')
      }
    }
      ...
      if (is.null(control$verbose)) {
        fit_2 <- quiet(
          poEMirtbase_gibbs_fit(
            Y = fit$info$data$response,
            N = fit$info$data$trial,
  ...
  class(L) <- paste0('poEMirt', ifelse(method == 'bootstrap', 'Boot', 'Gibbs'))
